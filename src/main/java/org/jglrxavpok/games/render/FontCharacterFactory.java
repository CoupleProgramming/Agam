...
public class FontCharacterFactory {
	...
	private HashMap<Character, IDrawingPixmap> characterCache = new HashMap<Character, IDrawingPixmap>();
	...
	public IDrawingPixmap getFontCharacter(char character, IDrawingCanvas screen) {
		if(character ==' ')
		{
			return new DrawingPixmap(6,6); // Retourne un caractère 'vide' si on veut un espace
		}
		...
		IDrawingPixmap characterBitmap = screen.createBitmap(pixels);
		characterCache.put(character, characterBitmap);
		
		return characterBitmap;
	}

	public int getHeightOffset(char character, IDrawingCanvas screen) {
		if (!characterHeightOffset.containsKey(character)) {
			getFontCharacter(character, screen);
		}
