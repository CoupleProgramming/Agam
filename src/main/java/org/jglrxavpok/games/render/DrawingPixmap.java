...
import org.jglrxavpok.games.Rectangle;

import java.io.Serializable;
import java.util.Arrays;

public class DrawingPixmap implements IDrawingPixmap, Serializable {

	private static final long	serialVersionUID	= 1173010494969338151L;
	protected int[] pixels;
	protected int w, h;

	public DrawingPixmap()
	{
		this(1,1);
	}
	public DrawingPixmap(int w, int h) {
		this.w = w;
		this.h = h;
		pixels = new int[w * h];
	}

	public DrawingPixmap(int w, int h, int[] pixels) {
		this.w = w;
		this.h = h;
		this.pixels = pixels;
	}

	public DrawingPixmap(int[][] pixels2D) {
		w = pixels2D.length;
		if (w > 0) {
			h = pixels2D[0].length;
			pixels = new int[w * h];
	...
	public IDrawingPixmap copy() {
		DrawingPixmap rValue = new DrawingPixmap(this.w, this.h);
	...
	public void blit(IDrawingPixmap bitmap, int x, int y) {
		...
		Rectangle blitArea = new Rectangle(x, y, mb.w, mb.h);
	...
	public void blit(IDrawingPixmap bitmap, int x, int y, int width, int height) {
		DrawingPixmap mb = (DrawingPixmap) bitmap;
		Rectangle blitArea = new Rectangle(x, y, width, height);
		adjustBlitArea(blitArea);
		int blitWidth = blitArea.bottomRightX - blitArea.topLeftX;
	...
	public void colorBlit(IDrawingPixmap bitmap, int x, int y, int color) {
		...
		for (int yy = blitArea.topLeftY; yy < blitArea.bottomRightY; yy++) {
			...
			int sp = (yy - y) * mb.w + (blitArea.topLeftX - x);
			for (int xx = 0; xx < blitWidth; xx++) {
				int col = ((DrawingPixmap) bitmap).pixels[sp + xx];
				if (col < 0) {
					int r = (col & 0xff0000);
					int g = (col & 0xff00);
					int b = (col & 0xff);
	...
	public void alphaFill(int x, int y, int width, int height, int color, int alpha) {
		...
		DrawingPixmap bmp = new DrawingPixmap(width, height);
	...
	private void adjustBlitArea(Rectangle blitArea) {
	...
	public IDrawingPixmap shrink() {
		DrawingPixmap newbmp = new DrawingPixmap(w / 2, h / 2);
		int[] pix = pixels;
		int blarg = 0;
	...
	public IDrawingPixmap scaleBitmap(int width, int height) {
		DrawingPixmap scaledBitmap = new DrawingPixmap(width, height);
